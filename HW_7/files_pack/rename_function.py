...
def rename_files(new_name: str, ext_renamed: str, /, count_dig: int = 3, ext_new: str = None,
                       saved_range: range = (3, 6), path: str = None) -> int:
    """Переименование файлов. Возвращает количество переименованных файлов.
    """
    if ext_new is None:  # Если не указано, сохранить текущее расширение
        ext_new = ext_renamed
    work_path = Path.cwd() if path is None else Path(
        path)  # Рабочий каталог - текущий, если не указано
    count_renamed = 0
