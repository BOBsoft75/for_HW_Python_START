...
def jsn_file_write(data_dict: dict, path: str, file_name: str) -> None:
...
def csv_file_write(data_dict: dict, path: str, file_name: str) -> None:
    file_path = os.path.join(path, file_name + '.csv')
    data = [['Path', 'object_type',  'object_name',
             'object_size', 'parent_directory']]
...
# запись словаря в pickle-файл
def pcl_file_write(data_dict: dict, path: str, file_name: str) -> None:
    file_path = os.path.join(path, file_name + '.pickle')
    with open(file_path, 'wb') as f:
        pickle.dump(data_dict, f)


def dct_format(total_dct: dict[str, dict[str]], path: str, item_name: str, item_type: str) -> None:
    if item_type == 'F':
        total_dct[path] = dict(object_type='File',
                               object_name=item_name,
                               object_size=os.path.getsize(
                                   os.path.join(path, item_name)),
                               parent_directory=os.path.split(path)[-1])
    elif item_type == 'D':
        total_dct[path] = dict(object_type='Directory',
                               object_name=item_name,
                               object_size=count_size(
                                   os.path.join(path, item_name)),
...
def directory_walk(aim_path: str, total_dct: dict = None) -> dict[str, dict[str]]:
    if total_dct is None:
        total_dct = {}
        basic_path = os.path.split(os.path.abspath(aim_path))

        dct_format(total_dct, os.path.join(
            *basic_path[:-1]), basic_path[-1], 'D')

    for item in os.listdir(aim_path):
        check_path = os.path.join(aim_path, item)
        if os.path.isfile(check_path):
            dct_format(total_dct, aim_path, item, 'F')
        elif os.path.isdir(check_path):
            dct_format(total_dct, aim_path, item, 'D')
            directory_walk(os.path.join(aim_path, item), total_dct)
    return total_dct


def dict_print(in_dict: dict) -> None:
...
def main():
    ...
    test_path = '\\GeekBrains\\PythonDeep(Seminars)\\for_HW_Python_START\\HW_8\\'
    result = directory_walk(test_path)
    jsn_file_write(result, os.getcwd(), 'result1')
    pcl_file_write(result, os.getcwd(), 'result1')
    csv_file_write(result, os.getcwd(), 'result1')
    # dict_print(result)
